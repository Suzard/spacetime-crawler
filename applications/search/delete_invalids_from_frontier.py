...
@Deleter(SkayaniEdwardc6ForsterjLink)
@Getter(SkayaniEdwardc6ForsterjLink)
class DeleteFrontierFrame(IApplication):
    ...
    def update(self):
        ...
        ls = self.frame.get(SkayaniEdwardc6ForsterjLink)
        print "Found ", len(ls), " links."
        count = 0
        for l in ls:
            if not is_valid(l.full_url):
                self.frame.delete(SkayaniEdwardc6ForsterjLink, l)
                count += 1
...
class Simulation(object):
    ...
    def __init__(self, address, port):
        ...
        objectless_connector = ObjectlessSpacetimeConnection(
            "ResetFrontier_SkayaniEdwardc6Forsterj".format(CrawlerFrame.app_id),
